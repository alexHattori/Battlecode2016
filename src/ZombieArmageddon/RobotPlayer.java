...
public class RobotPlayer {
	...
	private static void runArchon() throws GameActionException{
		if(rc.isCoreReady()){
			...
			if(!hasTurret && rc.getRoundNum()>10){
					...
					if(rc.isWeaponReady()){
						...
						for(RobotInfo a:enemies){
							if(foundBigZombie){
								...
								else if(a.type==RobotType.RANGEDZOMBIE){
									foundRanged = true;
									weakestEnemy = a;
								}
								else if(a.type==RobotType.ZOMBIEDEN && a.health<weakestEnemy.health){
									weakestEnemy = a;
								}
								...
								else if(a.type == RobotType.ZOMBIEDEN){
									foundDen = true;
									weakestEnemy = a;
								}
	...
	private static void runScout() throws GameActionException{
		...
		case LURING:
			...
			RobotInfo[] dangers = rc.senseHostileRobots(myLoc, 25);
